...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    softmax_new = softmax(np.dot(outside_vectors, center_word_vec))
    loss = -np.log(softmax_new[outside_word_idx])

    softmax_new[outside_word_idx] -= 1
    grad_center_vec = np.dot(outside_vectors.T, softmax_new)

    grad_outside_vecs = np.dot(np.expand_dims(softmax_new, axis = 1), np.expand_dims(center_word_vec, axis = 0))
    ### END YOUR CODE
...
def neg_sampling_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    dot_one = outside_vectors[outside_word_idx].T.dot(center_word_vec)
    neg_samples = outside_vectors[neg_sample_word_indices] 
    dot_two = -neg_samples.dot(center_word_vec)
    loss = -np.log(sigmoid(dot_one)) - np.sum(np.log(sigmoid(dot_two)))
    
    grad_center_vec = -(1 - sigmoid(dot_one)) * outside_vectors[outside_word_idx] + np.sum(
        (np.expand_dims(1 - sigmoid(dot_two),axis=1) * neg_samples), axis=0)

    grad_outside_vecs = np.zeros(shape=outside_vectors.shape)
    grad_outside_vecs[outside_word_idx] = -(1 - sigmoid(dot_one)) * center_word_vec
    for i,neg_idx in enumerate(neg_sample_word_indices):
        grad_outside_vecs[neg_idx] += (1 - sigmoid(dot_two)[i]) * center_word_vec
    ### END YOUR CODE
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for word in outside_words:
        loss_gradients = list(word2vec_loss_and_gradient(
            center_word_vectors[word2ind[current_center_word]], word2ind[word], outside_vectors, dataset))
        loss += loss_gradients[0] 
        grad_center_vecs[word2ind[current_center_word]] += loss_gradients[1]
        grad_outside_vectors += loss_gradients[2]
