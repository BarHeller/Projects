    ...
    while not it.finished:
        ...
        assert_allclose(numgrad, grad[ix], rtol=1e-5,
                        err_msg=f"Gradient check failed for {gradient_text}.\n"
                                ...
                                f"Your gradient: {numgrad} \t Numerical gradient: {grad[ix]}")
...
def test_gradcheck_basic():
    ...
    gradcheck_naive(quad, np.array(123.456), "scalar test")       # scalar test
    gradcheck_naive(quad, np.random.randn(3,), "1-D test")     # 1-D test
    gradcheck_naive(quad, np.random.randn(4, 5), "2-D test")   # 2-D test
    print()
