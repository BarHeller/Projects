...
class NerBiLstmModel(torch.nn.Module):
    ...
    def __init__(self, helper, config, pretrained_embeddings):
        ...
        self.embedding = torch.nn.Embedding.from_pretrained(pretrained_embeddings, freeze=False)
        self.blstm = torch.nn.LSTM(input_size=config.n_features*config.embed_size, hidden_size=int(config.hidden_size/2), batch_first=True, bidirectional=True)
        self.lin = torch.nn.Linear(config.hidden_size, config.n_classes)
        ### END YOUR CODE
    ...
    def forward(self, sentences):
        ...
        embedded_sent = self.embedding(sentences)
        embedded_sent = embedded_sent.view(batch_size, seq_length, -1)
        dropout_out = self._dropout(embedded_sent)
        bilstm_out = self.blstm(dropout_out)
        dropout_out_bilstm = self._dropout(bilstm_out[0])
        linear_out = self.lin(dropout_out_bilstm)
        softmax = torch.nn.Softmax(dim = 2)
        tag_probs = softmax(linear_out)
        ### END YOUR CODE
        return tag_probs
...
class Trainer(TrainerBase):
    def __init__(self, model, config, helper, logger):
        ...
        self._loss_function = torch.nn.NLLLoss(reduction='sum')
        ### END YOUR CODE
    ...
    def _batch_loss(self, tag_probs, labels, masks):
        ...
        masks_expanded = masks.unsqueeze(-1).expand_as(tag_probs)
        tag_probs = torch.log(tag_probs)
        masked_tag_probs = tag_probs.masked_fill(~masks_expanded, 0)
        masked_tag_probs = masked_tag_probs.transpose(1,2)
        ### END YOUR CODE
    ...
    def pad_sequences(self, examples):
        ...
        for sentence, labels in examples:
            ...
            if max_length - len(sentence) <= 0:
                updated_sentence = sentence[:max_length]
                updated_labels = labels[:max_length]
            else:
                updated_sentence = sentence + [zero_vector] * (max_length - len(sentence))
                updated_labels = labels + [zero_label] * (max_length - len(sentence))
            masking = [False if i >= len(sentence) else True for i in range(max_length)]
            ret.append((updated_sentence, updated_labels, masking))
            ### END YOUR CODE
        return ret
