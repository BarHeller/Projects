...
def replace_word(word):
    ...
    """
    ### YOUR CODE HERE
    # Numbers with , - remove the ,s and continue as number
    if re.search("^[+-]?\d{1,3},\d\d\d(,\d\d\d)*(.\d+){0,1}$", word):
        word = word.replace(",","")
    # Numbers - negative, positive, digits
    if re.search("^[+-]?\d+$", word):
        number = int(word)
        if number < 0: 
            return "NEGATIVE"
        ...
        if number < 1000:
            return "SMALL_NUMBER"
        return "LARGE_NUMBER"
    # Thousends and milions:
    if re.search("^\d+[K,M]$", word):
        return "LARGE_NUMBER"
    # Precentages (x%)
    if re.search("^\d*%$", word):
        return "PRECENTAGE"
    if re.search(r"^[-+]?\d*\.\d+$", word):
        number = float(word)
        # Probabilities ([0,1] real numbers)
        if 0 <= number and number <=1:
            return "PROBABILITY"
        # regular float
        if 0 < number:
    ...
    if re.search("^(\d\d:)?\d\d:\d\d$", word):
        return "TIME"
    # Date (dd/mm[/yy[yy]], dd-mm[-yy[yy]]) or date range
    if re.search("^(\d{1,2}-)?\d{1,2}[/-]\d{1,2}([/-](\d\d){1,2}){0,1}$", word):
        return "DATE"
    # Date ([yy[yy]/]mm/dd, [yy[yy]-]mm-dd) or date range
    if re.search("^((\d\d){1,2}[/-])?\d{1,2}[/-](\d{1,2}-){0,1}\d{1,2}$", word):
        return "DATE"
    # Counts (x1st, x2nd, x3rd,xth)
    if re.search("^\d?1st$", word) or \
        re.search("^\d?2nd$", word) or \
        re.search("^\d?3rd$", word) or \
        re.search("^\d\d*th$", word):
        return "COUNT"
    if re.search("ly$", word.lower()):
        return "LY";
    if word.isalpha() and word.upper() == word:
        return "ALL_CAPS"
    # number - word
    if re.search("^[-+]?\d*(\.\d+){0,1}(-[A-Za-z]+)+$", word):
        return "NUMBER_WORD"
    if re.search("^[A-Za-z]+-[-+]?\d*(\.\d+){0,1}$", word):
        return "WORD_NUMBER"
    # word - word [(- word)*]
    if re.search("^[a-zA-Z]+(-[a-zA-Z]+)+$", word):
        return "WORD_WORD"
    if re.search("^([-+]?\d*(\.\d+){0,1})+(-([-+]{0,1})\d*(\.\d+){0,1})+$", word):
        return "NUMBER_NUMBER"
    if re.search("^[-+]?\d*(\.\d+){0,1}(:[-+]{0,1}\d*(\.\d+){0,1})+$", word):
        return "NUMBER_COLON_NUMBER"
    # Entity (starts with Cap)
    if re.search("^[A-Z][a-z]+$", word):
        return "Entity"
    if re.search("^[a-zA-Z]+ed$", word.lower()):
        return "ED"
    if re.search("^[a-zA-Z]+al$", word.lower()):
        return "AL"
    ### END YOUR CODE
    return "UNK"
