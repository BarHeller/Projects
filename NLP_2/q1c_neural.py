...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    gradW2 = np.dot(np.transpose(h), y_hat - y)

    gradient_1 = np.dot(y_hat - y, np.transpose(W2))
    gradient_1 *= sigmoid_grad(h)

    gradb1 = np.sum(gradient_1, axis = 0, keepdims = True)

    gradW1 = np.dot(np.transpose(data), gradient_1)

    gradb2 =  np.sum(y_hat - y, axis = 0, keepdims = True)
