...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    for i in range(BATCH_SIZE):
        random_index = np.random.randint(len(in_word_index))
        data[i] = num_to_word_embedding[in_word_index[random_index]]
        labels[i][out_word_index[random_index]] = 1
...
def eval_neural_lm(eval_data_path):
    ...
    for i in range(num_of_examples):
        cur_in_word = num_to_word_embedding[in_word_index[i]]
        prob_of_correct_out = forward(cur_in_word, out_word_index[i], params, dimensions)
        log_sum += np.log2(prob_of_correct_out)
    l = log_sum/num_of_examples
